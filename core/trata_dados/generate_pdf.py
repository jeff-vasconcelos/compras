...
def pdf_ruptura(request):
    id_empresa = request.user.usuario.empresa_id
    estado = ["PARCIAL", "RUPTURA"]

    itens = Alerta.objects.filter(empresa__id=id_empresa, estado_estoque__in=estado)

    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'inline; filename="Relatório-Rupturas.pdf"'
    # response['Content-Disposition'] = 'attachment: filename="PedidoInsight.pdf"'
