...
from core.views.utils.datas import data_mes, get_days_in_period
...
def excesso_fornecedor(request):
    ...
    df = pd.DataFrame(
        Alerta.objects.filter(empresa__id__exact=empresa, estado_estoque="EXCESSO")
        .order_by("cod_fornecedor")
        .values()
    )
    ...
    if not df.empty:
        ...
        por_fornecedor = (
            df.groupby(["cod_fornecedor", "fornecedor", "cod_filial"])[list_val_cus]
            .sum()
            .round(2)
            .reset_index()
        )
        ...
        result = result.drop_duplicates(
            subset=["cod_fornecedor", "cod_filial"], keep="first"
        )

        p_fornec = result.to_dict("records")

        context = {"fornecedores": p_fornec, "valor_total_excesso": valor_total_excesso}

        return render(
            request, "aplicacao/paginas/fornecedor/excesso_fornec.html", context
        )
    else:
...
def ver_excesso_fornecedor(request, cod_fornecedor, filial):
    empresa = request.user.usuario.empresa_id
    fornec = Fornecedor.objects.get(
        empresa_id=empresa, cod_fornecedor__exact=cod_fornecedor
    )
    ...
    context = {"produtos": produtos_fornec, "fornecedor": fornec}

    return render(
        request, "aplicacao/paginas/fornecedor/ver_excesso_fornec.html", context
    )
...
def ruptura_fornecedor(request):
    ...
    df = pd.DataFrame(
        Alerta.objects.filter(empresa__id__exact=empresa, estado_estoque__in=estado)
        .order_by("cod_fornecedor")
        .values()
    )
    ...
    if not df.empty:
        ...
        por_fornecedor = (
            df.groupby(
                ["cod_fornecedor", "fornecedor", "cod_filial", "estado_estoque"]
            )[list_val_cus]
            .sum()
            .round(2)
            .reset_index()
        ...
        context = {"fornecedores": p_fornec}

        return render(
            request, "aplicacao/paginas/fornecedor/ruptura_fornec.html", context
        )
    else:
...
def ver_ruptura_fornecedor(request, cod_fornecedor, filial):
    ...
    return render(
        request, "aplicacao/paginas/fornecedor/ver_ruptura_fornec.html", context
    )
...
def graficos_alert_fornec(request):
    if request.is_ajax():
        try:
            ...
            # Getting Analisis date's
            parametros = Parametro.objects.get(empresa_id=id_empresa)
            periodo = parametros.periodo
            df_dates = get_days_in_period(periodo)

            # Getting Analsis products
            df_vendas, info_produto = vendas(
                cod_produto, id_empresa, periodo, [cod_filial]
            )
            ...
            # grafico
            graficos = graficos_serie(df_vendas, df_dates, df_historico)
            ...
            total_venda["mes"] = total_venda["data"].map(
                lambda x: 100 * x.year + x.month
            )
            total_vendas = (
                total_venda.groupby(["mes", "cod_filial"])["qt_vendas"]
                .sum()
                .reset_index()
            )
