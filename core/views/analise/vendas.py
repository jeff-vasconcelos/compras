...
from core.views.utils.datas import get_days_in_period
import pandas as pd
import datetime
...
def vendas(cod_produto, id_empresa, periodo, lista_filiais):
    ...
    data_fim = data_inicio - datetime.timedelta(
        days=periodo - 1
    ...
    for filial in lista_filiais:
        ...
        produto_qs = Produto.objects.get(
            cod_produto__exact=cod_produto, empresa__id__exact=id_empresa
        )
    ...
    for i in list:
        if i:
            ...
            preco = (
                vendas_df.groupby(["data"])["preco_unit"]
                .mean()
                .round(2)
                .to_frame()
                .reset_index()
            ...
            custo = (
                vendas_df.groupby(["data"])["custo_fin"]
                .mean()
                .round(2)
                .to_frame()
                .reset_index()
            ...
            qtvendas_preco_custo = pd.merge(
                qtvendas, preco_custo, how="left", on=["data"]
            )

            # MERGE VENDAS X DATAS
            vendas_datas = pd.merge(
                datas, qtvendas_preco_custo, how="left", on=["data"]
            )

            vendas_datas = vendas_datas.drop_duplicates(
                subset=["data", "cod_produto", "cod_filial"], keep="first"
            )
            ...
            values = {
                "cod_produto": produto_qs.cod_produto,
                "cod_filial": filial_cod,
                "cod_fornecedor": produto_qs.cod_fornecedor,
                "qt_vendas": 0,
                "custo_fin": 0,
                "preco_unit": 0,
            ...
            df_historico = historico_estoque(
                cod_produto, id_empresa, periodo, lista_filiais
            )
            ...
            lista_for_df = pd.DataFrame(
                data=lista_media, columns=["valores"]
            ).reset_index()
            ...
            info_p = {
                "cod_filial": filial_cod,
                "dias_s_vendas": [d_sem_vendas],
                "dias_vendas": [d_vendas],
                "media": [media],
                "maximo": [maximo],
                "desvio": [d_padrao.round(2)],
            ...
            vendas_lucro["vl_total_vendido"] = (
                e_vendas["qt_vendas"] * e_vendas["preco_unit"]
            )
            vendas_lucro["vl_total_custo"] = (
                e_vendas["qt_vendas"] * e_vendas["custo_fin"]
            )

            lucro = (
                vendas_lucro.groupby(["data"])[list_val_cus]
                .sum()
                .round(2)
                .reset_index()
            )
            e_vendas["lucro"] = (
                vendas_lucro["vl_total_vendido"] - vendas_lucro["vl_total_custo"]
            )
            ...
            _vendas = _venda.assign(
                **_venda.select_dtypes(["datetime"]).astype(str).to_dict("list")
            ).to_dict("records")
        ...
        if not duplicados.empty:
            informacao_produto = informacao_produto.drop_duplicates(
                subset=["cod_filial"], keep="first"
            )
